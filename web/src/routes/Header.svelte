<script>
  ...
  $: pgPath = $page.data.pathname.split('/').filter(a => a)
</script>
...
<header bind:this={header} class:active={isActive}>
  <div class="flex justify-end gap-6 w-limit single-line top-bar pc">
    ...
    <div class="divider single-line">
      <a data-sveltekit-reload href="/ko" class:selected={$page.params.lang == 'ko'}>한</a>
      <a data-sveltekit-reload href="/eng" class:selected={$page.params.lang == 'eng'}>A</a>
    </div>
  </div>
  <div class="flex w-limit gnb pc">
    ...
    <div class="flex items-center justify-end flex-auto menu gap-9">
      {#each menus as menu}
        <a href={menu.link} class="text-center menu-item" class:selected={menu.slug == (pgPath[1] ?? '/')}
